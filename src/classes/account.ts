...
export default class Account {
	...
	public get transactions(): any {
		...
		return this.util.db.accounts.r.table("Transactions").filter(this.util.db.accounts.r.row("fromID").eq(this.id).or(this.util.db.accounts.r.row("toID").eq(this.id))).default([]);
	}
	...
	public setBalance(amount: number): Account {
		...
		this.util.db.accounts.r!.table("Accounts").get(this.id).update({ _balance: this._balance });
		return this;
	}

	public setLanguage(lang: keyof typeof Languages) {
		this._language = lang;
		this.util.db.accounts.r!.table("Accounts").get(this.id).update({ _language: this._language });
	}

	public setPrefix(prefix: string | null): Account {
		if (prefix === null) this._prefix = settings.prefix;
		else this._prefix = prefix;
		this.util.db.accounts.r!.table("Accounts").get(this.id).update({ _prefix: this._prefix });
		return this;
	}
