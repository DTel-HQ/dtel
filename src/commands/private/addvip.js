module.exports = async(client, msg, suffix) => {
	...
	msg.channel.send({ embed: { color: config.colors.receipt, title: `${added ? "Added" : "Removed"} VIP Months!`, description: `${added ? "Added" : "Removed"} \`${added ? months : months * -1}\` VIP Months ${added ? "to" : "from"} ${user} (${user.id}).`, footer: { text: `${msg.author.id}`, icon_url: msg.author.displayAvatarURL() } } });
	...
	else (await user.createDM()).send({ embed: { color: config.colors.receipt, title: "VIP months have been removed", description: `\`${months * -1}\` VIP months were removed from your account.`, footer: { text: `${msg.author.tag}`, icon_url: msg.author.displayAvatarURL() } } }).catch(e => msg.channel.send({ embed: { color: config.colors.error, title: "Private DMs", description: "Could not send the user their receipt in DMs." } }));
};
