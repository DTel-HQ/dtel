module.exports = async(client, msg, suffix, call) => {
	...
	await client.apiSend({ embed: { color: config.colors.error, title: "The call has ended!", description: `The other side ended the call${call.pickedUp ? ` after ${time}` : ""}.`, footer: { text: call.id } } }, msg.channel.id === call.from.channel ? call.to.channel : call.from.channel).catch(_ => null);
	await r.table("OldCalls").insert(call);
	await client.log(`:negative_squared_cross_mark: ${call.rcall ? "rcall" : "Call"} \`${call.from.hidden ? "hidden" : call.from.channel} → ${call.to.hidden ? "hidden" : call.to.channel}\` was hung up by \`${hidden ? "Anonymous#0000" : msg.author.username}\` (${hidden ? "hidden" : msg.author.id}) on the ${msg.channel.id === call.from.channel ? "from" : "to"} side. ${call.id}`);
