...
if (!config.devMode) {
	scheduleJob("*/1 * * * *", async() => {
		...
		try {
			updateResults = await fetch("https://discord.austinhuang.me/dtel", {
				method: "GET",
				headers: {
			...
			winston.error(
				`Yo, there might be something wrong with the votes API.\n\`\`\`\n${err}\n\`\`\``,
				err,
			);
			return;
		}
		...
		for (const id of userIDs) {
			...
			const dmEmbed = new EmbedBuilder()
				...
				.setDescription(
					`You received ${config.dtsEmoji}${responseBody[user.id]} for voting!`,
				)
				.setAuthor({
...
scheduleJob("0 20 * * 0", async() => {
	...
	if (topVoters.length === 0) {
		return client.log(
			`<:blobsad:386228996486070272> No qualifying voters this week.`,
		);
	}
	...
	for (const voteDescription of topVoters) {
		...
		const winningEmbed = new EmbedBuilder()
			...
			.setDescription(
				`You have received ${prizeMonths} VIP Month(s) for being ${
					topVoters.length === 1 ? "the" : "a"
				} highest voter this week.`,
			)
			.setFooter({
	...
	const announcementEmbed = new EmbedBuilder()
		...
		.setDescription(
			`The voter(s) who voted the most have been awarded ${prizeMonths} VIP month${
				prizeMonths == 1 ? "" : "s"
			}.`,
		)
		.addFields([
	...
	try {
		const res = await client.sendCrossShard(
			{ embeds: [announcementEmbed] },
			config.supportGuild.channels.announcement,
		);
		await client.rest.post(
			`/channels/${config.supportGuild.channels.announcement}/messages/${res.id}/crosspost`,
		);
	} catch (e) {
		console.error(e);
		client.sendCrossShard(
...
scheduleJob("0 0 0 * * *", async() => {
	...
	const calls = (
	(await db.archivedCalls.aggregateRaw({
		pipeline: [
			{ $match: { ended: { at: {
				$gt: beginDate	},
			},
	...
	const result = await db.callMessages.deleteMany({ where: {
		callID: {
			in: calls,
		},
	},
