...
module.exports = async(client, message, args, callDocument) => {
  ...
  else if (!callDocument.pickedUp) return message.reply(":x: You can't transfer a pending call!");
  let mynumber;
	try {
  ...
  let toDial = args;
  if (!toDial) return message.reply(":thinking: Wanna transfer a call to void? How about `>hangup`?");
  let toDialDocument;
  ...
  toDial = toDial.replace(/(a|b|c)/ig, "2")
    ...
    .replace(")", "")
    .replace(/\s+/g, "");
  if (toDial === mynumber.number) return message.reply(":thinking: Why are you trying to transfer it to yourself?");
  if (toDial === "*611") {
  ...
  if (toDial === "*411" || toDial === "*233") return message.reply("You can't transfer to `*411` or `*233`.");
  try {
  ...
  if (callDocument.to.channelID === message.channel.id) {
    ...
    callDocument.from = {channelID: toDialDocument._id, number: toDialDocument.number};
  }
