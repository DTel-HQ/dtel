...
module.exports = async(bot, message, args) => {
	...
	if (userid === message.author.id && !bot.guilds.get(process.env.SUPPORTGUILD).members.get(message.author.id).roles.find("name", "Boss")) {
	...
	bot.channels.get(process.env.LOGSCHANNEL).send(`:money_with_wings: Support member ${message.author.username} added ¥${amount} to <@${userid}>.`);
};
