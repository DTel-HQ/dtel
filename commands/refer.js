var fs = require("fs"),
    ...
    referrals = JSON.parse(fs.readFileSync("././json/refers.json", "utf8")),
	referreds = JSON.parse(fs.readFileSync("././json/guilds.json", "utf8"));
...
exports.run = (bot, message, args) => {
	if (args[1] === undefined) {
		...
		if (message.guild.owner.id !== message.author.id) {
			...
			var leaccount = accounts.find(item => {return item.user === args[1];});
