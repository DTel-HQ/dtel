...
exports.run = (bot, msg, args) => {
			...
			if(!mailbox){
				...
				fs.writeFile("mailbox.json",JSON.stringify(mailbox_storage), "utf8");
			}
			switch(msg.content.split(" ")[1]){
				...
				if(!msg.content.split(" ")[2]){
					msg.channel.send({embed:{title: ":mailbox_with_mail: You have "+mailbox.messages.length+" messages.", fields: mailbox.messages.map(m=>{
						return {name: "ID \""+m.id+"\" from "+m.from, value: m.message};
					}), footer: {text: "Message options: `>mailbox messages [id]`"}}});
				} else {
