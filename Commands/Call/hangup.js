module.exports = async(client, msg, suffix, call) => {
	...
	// Send the stuff
	msg.channel.send({ embed: { color: config.colors.info, title: "The call has ended!", description: `You have ended the call${call.pickedUp ? ` after ${time}` : ""}.` } });
	client.apiSend({ embed: { color: config.colors.info, title: "The call has ended!", description: `The other side ended the call${call.pickedUp ? ` after ${time}` : ""}.` } }, msg.channel.id === call.from.channel ? call.to.channel : call.from.channel);
