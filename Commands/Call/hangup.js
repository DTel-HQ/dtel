module.exports = async(client, msg, suffix, call) => {
	...
	if (call.to.channel === config.supportChannel || call.from.channel === config.supportChannel) {
		const perms = [];
		for (let id of call.permissions) perms.push({ id: id, deny: ["SEND_MESSAGES"] });
		client.api.channels.cache.get(config.supportChannel).overwritePermissions(perms, `Call hung up (${call.id})`);
	}
};
