module.exports = async(client, msg, suffix, call) => {
	...
	client.log(`:white_check_mark: ${call.rcall ? "Rcall" : "Call"} \`${call.from.hidden ? "hidden" : call.from.channel} → ${call.to.hidden ? "hidden" : call.to.channel}\` was picked up by ${call.to.hidden ? "Anonymous" : msg.author.tag} (${call.to.hidden ? "hidden" : msg.author.id}). ${call.id}`);
