module.exports = async(client, msg, suffix, call) => {
	...
	if (call.to.number === config.supportNumber) {
		...
		await channel.overwritePermissions(
			...
			"*611 call pickup");
		await r.table("Calls").get(call.id).update({ permissions: [msg.author.id] });
	}

	let channelFrom = await client.channels.fetch(call.from.channel);

	// Pickup - reply first or it'll seem slow
	if (call.channel.to !== config.supportChannel) msg.channel.send({ embed: { color: config.colors.success, title: "You picked up the call.", description: "You can now put the call on `>hold`, or transfer the call to another number by using `>transfer <number>`! Note: if they are trolls, call *611.", footer: { text: call.id } } });
