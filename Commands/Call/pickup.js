module.exports = async(client, msg, suffix, call) => {
	...
	if (msg.channel.id !== config.supportChannel) msg.channel.send({ embed: { color: config.colors.success, title: "You picked up the call.", description: "You can now put the call on `>hold`, or transfer the call to another number by using `>transfer <number>`! Note: if they are trolls, call *611.", footer: { text: call.id } } });
