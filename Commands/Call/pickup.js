module.exports = async(client, msg, suffix, call) => {
	...
	if (call.to.channel !== config.supportChannel) msg.channel.send({ embed: { color: config.colors.success, title: "You picked up the call.", description: "You can now put the call on `>hold`, or transfer the call to another number by using `>transfer <number>`. Please remember to follow the [Rules](https://dtel.austinhuang.me/en/latest/FAQ/#rules).", footer: { text: call.id } } });
	...
	client.apiSend({ content: call.transferredBy ? "" : channelFrom.type === "dm" ? "" : `<@${call.startedBy}>`, embed: { color: config.colors.success, title: "The other side picked up!", description: "You can now put the call on `>hold`, or transfer a call to another number by using `>transfer <number>`. Please remember to follow the [Rules](https://dtel.austinhuang.me/en/latest/FAQ/#rules).", footer: { text: call.id } } }, call.from.channel);
