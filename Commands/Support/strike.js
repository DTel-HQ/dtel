...
module.exports = async(client, msg, suffix) => {
	...
	if (totalStrikes.length >= 3) {
		...
		if (!blacklist) {
			...
			client.log(`:cloud_lightning: ${channel ? `Guild` : `User`} ID \`${toStrike}\` has been stricken with ID ${strikeDoc.id} by \`${msg.author.tag}\` and was automatically blacklisted. (${totalStrikes.length} strikes)`);
			return;
		}
	}

	await msg.channel.send({ embed: { color: config.colors.success, title: "User striked", description: `**${user ? user.username : guild.name}** has been stricken and now has ${totalStrikes.length} ${totalStrikes.length == 1 ? "strike" : `strikes`}.\nStrike ID: \`${id}\`` }, author: { text: msg.author.tag, icon_url: msg.author.displayAvatarURL() } });
	client.log(`:cloud_lightning: ${channel ? `Guild` : `User`} ID \`${toStrike}\` has been stricken with ID ${strikeDoc.id} by \`${msg.author.tag}\`. (${totalStrikes.length} strikes)`);
	if (silent) return;
	if (user) {
		...
		(await guildOwner.createDM()).send({ embed: { color: config.colors.info, title: "Your server has received a srike", description: `Your server ${guild.name}(${guild.id}) has been striked due to the following reason: \n_${reason}_ \n\nYour server will be blacklisted after receiving ${3 - totalStrikes.length} more strikes.` } }).catch(e => msg.channel.send({ embed: { color: config.colors.error, title: "Error", description: "Couldn't reach the guild owner. Please manually inform them of the strike." } }));
		if (channel) channel.send({ embed: { color: config.colors.info, title: "This server has received a strike", description: `This server has been striked due to the following reason: \n_${reason}_ \n\nThe server will be blacklisteded after receiving ${3 - totalStrikes.length} more strikes.` } }).catch(e => msg.channel.send({ embed: { color: config.colors.error, title: "Error", description: "Couldn't reach the channel. The guild owner should have been informed." } }));
	}
};
