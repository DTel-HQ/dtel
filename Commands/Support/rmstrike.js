module.exports = async(client, msg, suffix) => {
	...
	const strikeID = suffix.split(" ")[0];
	const silent = suffix.split(" ")[1] === "-s";
	...
	if (!silent) (await client.users.cache.get(strike.offender).createDM()).send({ embed: { color: config.colors.info, title: "A strike has been removed", description: `A customer support agent removed strike \`${strikeID}\` from your strikes. You now have ${totalStrikes} strike(s) and need ${3 - totalStrikes} more to get blacklisted.` } });
};
