module.exports = async(client, msg, suffix, guildID) => {
	...
	if (guildID) guild = await client.guilds.get(guildID);
	if (!guild) guild = await client.guilds.get(suffix.split(" ")[0]);
	if (!guild) guild = await client.guilds.get(suffix.split(" ")[1]);
	...
	if (userID) member = await guild.members.get(userID);
	if (!member) member = await guild.members.get(suffix.split(" ")[1]);
	if (!member) member = await guild.members.get(suffix.split(" ")[0]);
