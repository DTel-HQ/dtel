module.exports = async(client, msg, suffix) => {
	...
	if (!user || !months || months == 0) return msg.channel.send({ embed: { color: 0xEEEEEE, title: "Command usage", description: ">addvip [user] [months]" } });
	...
	msg.channel.send({ embed: { color: 0xEEEEEE, title: "Succes", description: `${Number(months) > 0 ? "Added" : "Removed"} \`${Number(months) > 0 ? months : months * -1}\` VIP Months to ${user}(${user.id})`, footer: { text: `Executed by: ${msg.author}(${msg.author.id})` } } });
	if (Number(months) > 0) (await user.createDM()).send({ embed: { color: 0xEEEEEE, title: "Thanks for your purchase!", description: `You have received \`${months}\` VIP Months.`, footer: { text: `Executed by: ${msg.author.tag}(${msg.author.id})` } } }).catch(e => null);
	else (await user.createDM()).send({ embed: { color: 0xEEEEEE, title: "VIP Months have been removed", description: `There have been \`${months * -1}\` VIP Months removed from your account.`, footer: { text: `Executed by: ${msg.author.tag}(${msg.author.id})` } } }).catch(e => null);
};
