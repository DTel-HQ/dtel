...
module.exports = async(client, msg, suffix, rcall) => {
	...
	const fromBlocked = id => myNumber.blocked && myNumber.blocked.includes(id);
	const toBlocked = id => toDialDoc.blocked && toDialDoc.blocked.includes(id);
	if (fromBlocked(toDialDoc.id) || (toDialDoc.guild ? fromBlocked(toDialDoc.guild) : fromBlocked(toDialDoc.owner))) return msg.channel.send({ embed: { color: config.colors.error, title: "Dialing error", description: "You have blocked this number." } });
	...
	client.apiSend({ content: toDialDoc.mentions ? toDialDoc.mentions.join(" ") : "", embed: { color: callDoc.from.vip ? config.colors.vip : config.colors.info, title: "Incoming call", description: `There is an incoming call from ${myNumber.id === config.supportNumber ? "Customer Support" : myNumbervip ? myNumber.vip.hidden ? myNumber.vip.name ? `\`${myNumber.vip.name}\`` : "hidden" : myNumber.vip.name ? `\`${myNumber.vip.name} (${myNumber.id})\`` : contact ? `:green_book:${contact.name}` : `\`${myNumber.id}\`` : contact ? `:green_book:${contact.name}` : `\`${myNumber.id}\``}. You can either type \`>pickup\` or \`>hangup\`, or wait it out.`, footer: { text: callDoc.id } } }, toDialDoc.channel === config.supportChannel ? config.fakeSupportChannel : toDialDoc.channel);

	if (csCall) client.channels.cache.get(config.fakeSupportChannel).id = config.supportChannel;
	...
	setTimeout(async() => {
		...
		if (!newCallDoc || newCallDoc.pickedUp || newCallDoc.hungupBy) return;
