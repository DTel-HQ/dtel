...
module.exports = async(client, msg, suffix, rcall) => {
	...
	client.log(`:telephone: ${rcall ? "rcall" : "Call"} \`${myNumbervip && myNumber.vip.hidden ? "hidden" : myNumber.channel} → ${toDialvip && toDialDoc.vip.hidden ? "hidden" : toDialDoc.channel}\` has been established by \`${myNumbervip && myNumber.vip.hidden ? "Anonymous#0000" : msg.author.username}\` (${myNumbervip && myNumber.vip.hidden ? "hidden" : msg.author.id}). ${callDoc.id}`);
