	...
	if (!amount || !currency) {
		try {
			...
			currencies.forEach(curr => {
				...
				content.embed.fields.push({ name: `${emoji} [${curr.id}](https://dash.discoin.zws.im/#/currencies "${curr.name}")`, value: `${config.dtsEmoji}1 = ${emoji}${client.format(Math.round((dts.value / curr.value) * 100) / 100)}\n${emoji}1 = ${curr.value} D$`, inline: true });
			});
