	...
	if (!amount || !currency) {
		try {
			...
			currencies.forEach(curr => {
				...
				content.embed.fields.push({ name: `${emoji} ${curr.id} (~${client.format(curr.value)} D$)`, value: `${config.dtsEmoji}1 = ${emoji}${client.format(Math.round((dts.value / curr.value) * 100) / 100)}`, inline: true });
			});
